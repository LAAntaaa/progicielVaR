...
/* Textes de la sidebar — uniquement les éléments non-widgets */
[data-testid="stSidebar"] p,
[data-testid="stSidebar"] .stMarkdown p,
[data-testid="stSidebar"] label:not([data-baseweb]) {
    color: var(--txt-secondary) !important;
    font-size: 12px !important;
    font-family: var(--font-ui) !important;
}

/* ── Bouton réouverture sidebar — forcé visible en permanence ── */
[data-testid="stSidebarCollapsedControl"] {
    ...
    box-shadow: 4px 0 20px rgba(0,212,255,0.12) !important;
...
[data-testid="stSidebarCollapsedControl"]:hover {
    ...
    box-shadow: 4px 0 28px rgba(0,212,255,0.25) !important;
}
...
with st.sidebar:
    ...
    menu = st.radio(
        "Navigation",
        ["🏠  Accueil", "🏦  Portefeuille", "📐  Optimisation",
         "📉  Calcul VaR", "🧪  Backtesting", "🔥  Stress-Testing", "📊  Reporting"],
...
# ── Bouton sidebar : injection JS via components.html (seul moyen qui fonctionne)
components.html("""
<script>
(function() {
  function fix() {
    var doc = window.parent.document;
    // Tous les sélecteurs connus pour le bouton collapse/expand
    var selectors = [
      '[data-testid="stSidebarCollapsedControl"]',
    ...
    selectors.forEach(function(sel) {
      doc.querySelectorAll(sel).forEach(function(el) {
        ...
        el.style.setProperty('border', '1px solid rgba(0,212,255,0.4)', 'important');
        el.style.setProperty('border-left', 'none', 'important');
        el.style.setProperty('border-radius', '0 8px 8px 0', 'important');
        el.style.setProperty('box-shadow', '4px 0 20px rgba(0,212,255,0.15)', 'important');
        ...
        el.querySelectorAll('svg').forEach(function(svg) {
          svg.style.setProperty('color', '#00D4FF', 'important');
          svg.style.setProperty('fill', '#00D4FF', 'important');
        });
        el.querySelectorAll('button').forEach(function(btn) {
          btn.style.setProperty('opacity', '1', 'important');
          btn.style.setProperty('color', '#00D4FF', 'important');
        });
        el.onmouseenter = function() {
          el.style.setProperty('background', '#1a2235', 'important');
          el.style.setProperty('border-color', 'rgba(0,212,255,0.8)', 'important');
          el.style.setProperty('box-shadow', '4px 0 28px rgba(0,212,255,0.3)', 'important');
        };
        el.onmouseleave = function() {
          el.style.setProperty('background', '#111827', 'important');
          el.style.setProperty('border-color', 'rgba(0,212,255,0.4)', 'important');
          el.style.setProperty('box-shadow', '4px 0 20px rgba(0,212,255,0.15)', 'important');
        };
      });
    });
  }
  // Exécuter immédiatement et observer les mutations
  fix();
  setTimeout(fix, 300);
  setTimeout(fix, 1000);
