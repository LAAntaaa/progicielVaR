...
def telecharger(tickers: list, debut: str, fin: str) -> pd.DataFrame:
    if not HAS_YF:
        return pd.DataFrame()
    try:
        data = yf.download(tickers, start=debut, end=fin, auto_adjust=True, progress=False)
    except Exception:
        return pd.DataFrame()
    if data.empty:
        return pd.DataFrame()
    if isinstance(data.columns, pd.MultiIndex):
        prix = data["Close"].copy() if "Close" in data.columns.get_level_values(0)                else data.xs(data.columns.get_level_values(0)[0], axis=1, level=0)
    else:
    ...
    if isinstance(prix, pd.Series):
        prix = prix.to_frame()
    return prix.ffill().bfill().dropna(axis=1, how="all")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Sync pv immédiatement (sans attendre le bouton Charger)
        ...
        if len(actifs_choisis) < 2:
            ...
            with st.spinner("Chargement…"):
                if src == "Yahoo Finance" and HAS_YF:
                    prix = telecharger(tickers, str(debut), str(fin))
                    if prix.empty:
                        st.warning("Aucune donnée, simulation activée.")
                        prix = simuler(tickers)
                else:
                    prix = simuler(tickers)
                # Log-returns + ffill préventif
                prix_c = prix.ffill().bfill()
                rend = np.log(prix_c / prix_c.shift(1)).iloc[1:]
                # Seuil souple : supprimer seulement si > 5% NaN
                cols_ok = rend.columns[rend.isna().mean() < 0.05]
                if len(cols_ok) < 2:
                    cols_ok = rend.columns[rend.isna().mean() < 0.30]
                rend = rend[cols_ok].dropna()
                prix = prix_c[cols_ok]
                if rend.empty or len(rend.columns) < 2:
                    st.error("Données insuffisantes. Élargissez la période ou passez en Simulation.")
                    st.stop()
            suppr = len(tickers) - len(rend.columns)
            msg = f"✅ {len(rend.columns)} actif(s) — {len(rend)} jours"
            if suppr > 0:
                msg += f" ({suppr} actif(s) retirés)"
    ...
    if st.session_state["rendements"] is not None:
        ...
        port = rend.mean(axis=1).dropna()
        ann_r = port.mean()*252; ann_v = port.std()*np.sqrt(252)
        sh_   = ((port.mean()-0.03/252)/port.std()*np.sqrt(252)) if port.std() > 0 else 0.0
        cum   = (1+port).cumprod()
        mdd_  = float((cum/cum.cummax()-1).min()) if len(cum) > 0 else 0.0
        ...
        c5.metric("Skewness",  f"{stats.skew(port.dropna()):.4f}" if len(port.dropna())>3 else "—")
        c6.metric("Kurtosis",  f"{stats.kurtosis(port.dropna()):.4f}" if len(port.dropna())>3 else "—")
        ...
        df_s = pd.DataFrame({
            ...
            "Rdt (%)":  (rend.mean()*252*100).round(2).values,
            "Vol. (%)": (rend.std()*np.sqrt(252)*100).round(2).values,
            "Skewness": [_safe(stats.skew,     rend[col]) for col in rend.columns],
            "Kurtosis": [_safe(stats.kurtosis, rend[col]) for col in rend.columns],
            "Min (%)":  (rend.min()*100).round(3).values,
            "Max (%)":  (rend.max()*100).round(3).values,
        }, index=rend.columns)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.session_state["var_results"]:
        ...
        for i, (method, res) in enumerate(var_res.items()):
            ...
            vv  = 0.0 if not np.isfinite(vv)  else vv
            pv_ = 0.0 if not np.isfinite(pv_) else pv_
